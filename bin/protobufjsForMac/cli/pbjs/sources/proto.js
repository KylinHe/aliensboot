...
proto.load = function(filename, options, loaded) {
    ...
    if (Array.isArray(data['imports'])) {
        var newImports = new Array();
        var imports = data['imports'];
        for (var i=0; i<imports.length; i++) {
              //modify by jialin.he  skip server import
            if (imports[i] == "github.com/gogo/protobuf/gogoproto/gogo.proto" || imports[i]=="google/protobuf/timestamp.proto") {
                continue;
            } else {
                newImports.push(imports[i])
            }
